...
| Submit a task              | type text + Enter at the prompt | Begin an agent turn for that task.                                    || Show help                  | `/help`                         | List the available REPL commands.                              || Show context usage         | `/context`                      | Print prompt tokens used on the last LLM call vs. the context window. |
