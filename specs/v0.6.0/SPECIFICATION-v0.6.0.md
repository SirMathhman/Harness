...
1. **Two new subagent-side lifecycle hooks** (`subagent:turn:start`, `subagent:turn:end`) that fire on the _subagent's own_ hook manager, complementing the existing spawner-side `subagent:before` / `subagent:after`. Together, the four events give full lifecycle coverage around a subagent run.
